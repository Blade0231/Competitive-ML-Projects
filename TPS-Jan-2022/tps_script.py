...
def plot_imp(model):
    feature_imp = pd.Series(model.feature_importances_,index=training_cols).sort_values(ascending=False)
...
if __name__ == '__main__':
    ...
    df['change_logdiff'] = np.log(
            df['num_sold'] / df.groupby(list(group_cols))['num_sold'].shift()
    ).replace([np.inf, -np.inf, np.nan], 0.).astype(np.float32)

    training_cols = ['change_perc', 'change_logdiff',
       'country_Finland', 'country_Norway', 'country_Sweden',
       'store_KaggleMart', 'store_KaggleRama', 'product_Kaggle Hat',
       'product_Kaggle Mug', 'product_Kaggle Sticker', 'weekday', 'week_no',
       'month', 'Weekend', 'year']

    t_data, t_pred = prep_data(df, training_cols)

    X_train, X_test, y_train, y_test = train_test_split(t_data, t_pred, test_size=0.25, random_state=42)
